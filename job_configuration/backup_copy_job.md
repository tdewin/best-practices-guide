...
A copy job restarts at the defined **Copy every** interval setting (default is 12:00 AM daily) and looks for new restore points of the selected VMs in the specified sources. On the **Scheduler** tab, it is possible to schedule the when data transfers are allowed. This is especially helpful, when transferring multiple times per day (e.g. hourly synchronization interval).
...
2.  **Transform phase** — copy jobs are by nature running in "forever forward incremental" mode, and performs  transform operations on the target backup repository accordingly. Additionally, it is possible to schedule health checks or backup file compacting as described in the [Backup Job](./backup_job.md#storage-maintenance) section. The transform phase begins when all VMs are successfully copied to the target.
...
3.  **Idle phase** — for the most time, the backup copy job remains in the *Idle* state, waiting for a new restore point to appear on the source backup repository. When the synchronization interval expires steps 1-2 are repeated.
...
-   **Select from backup**: this method is equivalent to the **Select from job** method but allows for selecting specific VMs inside specific backups. This is helpful, when only certain critical VMs should be copied offsite.
